...
func BuildEAP5GANParameters() []byte {
	...
	log.Info("Mcc and Mnc In Octets - [ 0 ] : %x\r", resu[0])
	log.Info("Mcc and Mnc In Octets: %x\r", resu)

	log.Info("Build GUAMI - Init ")
	anParameter := make([]byte, 2)
	guami := make([]byte, 6)
	guami[0] = utils.ConvertToHexByte("2")  //0x02
	guami[1] = utils.ConvertToHexByte("f8") //0xf8
	guami[2] = utils.ConvertToHexByte("39") //0x39
