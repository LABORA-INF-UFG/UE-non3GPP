...
func DispatchNas(message []byte, ue *context.UeNas) ([]byte, error) {
	...
	if message == nil {
		log.Error("[UE][NAS] Invalid Message")
	}
	...
	if m.SecurityHeaderType != nas.SecurityHeaderTypePlainNas {
		...
		mac32, err := security.NASMacCalculate(ue.NasSecurity.IntegrityAlg,
			...
			payload)
		if err != nil {
			log.Error("[UE][NAS] Error in MAC verification")
			return nil, fmt.Errorf("NAS MAC calculate error")
		}

		// check integrity
		if !reflect.DeepEqual(mac32, macReceived) {
			log.Error("[UE][NAS] Error in MAC verification (received:", macReceived, "expected:", mac32, ")")
			return nil, fmt.Errorf("[UE][NAS] MAC verification failed(received:", macReceived, "expected:", mac32)
		} else {
			log.Info("[UE][NAS] Successful MAC verification")
		}
		...
		if cph {
			if err = security.NASEncrypt(ue.NasSecurity.CipheringAlg,
				...
				security.BearerNon3GPP, security.DirectionDownlink,
				payload[1:]); err != nil {
				log.Error("[UE][NAS] Error in Encryption MAC")
		...
		if err != nil {
			log.Error("[UE][NAS] Error in Decode NAS message")
